...
This example app now mirrors the same Twomilia configuration pattern used in FeedYum:
...
```bash
cd example
flutter run --dart-define=EXPO_PUBLIC_MOBILEAI_KEY=your_twomilia_key --dart-define=GEMINI_API_KEY=your_gemini_key
```

By default the sample talks to `https://twomilia.com`. For local backend
...
- The sample defaults `EXPO_PUBLIC_MOBILEAI_BASE_URL` to `https://twomilia.com`.
